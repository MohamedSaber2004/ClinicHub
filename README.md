...
| 5 | Doctor dashboard pages render inside the clinic layout for clinic owner | `_ViewStart.cshtml`, doctor views, `_ClinicLayout` | None (UI) |
...
## 5. Doctor dashboard inside the clinic owner dashboard

**What changed:** the clinic owner (who is also a doctor — الطبيب المسؤول) can now open the doctor
pages from the clinic sidebar, and the pages render **inside the clinic dashboard** (same sidebar/layout).

### How it works (frontend only)

- **New `Views/Doctor/_ViewStart.cshtml`** picks the layout from the logged-in profile role:
  - `ClinicOwner` / `ClinicManager` → `_ClinicLayout` (stays in the clinic dashboard)
  - `Doctor` → `_DoctorLayout` (pure doctors keep the doctor dashboard)
- The 6 doctor views no longer hardcode `Layout = "_DoctorLayout"`.
- `_ClinicLayout.cshtml` upgrades the sidebar context to the full clinic-owner menu when doctor pages
  are rendered inside it (the `DoctorController` mock context would otherwise shrink the menu).

### Sidebar (clinic owner)

The clinic sidebar now always shows a **لوحة الطبيب** section:
نظرة عامة (`/Doctor/Index`), مواعيدي (`/Doctor/Appointments`), مرضاي (`/Doctor/Patients`),
أوقات العمل المتاحة (`/Doctor/Availability`).

### Backend impact

- None — the doctor pages keep consuming the existing doctor dashboard endpoints
  (`GET /api/v1/doctors/availability`, appointments, patients, stats).
- **Review question for backend:** what is the `Role` returned by `GET /auth/profile` for a clinic owner
  who is also a doctor? The frontend uses `ClinicOwner`/`ClinicManager` to decide the layout.
...
- [ ] `GET /auth/profile` role for owner-doctors is `ClinicOwner` (frontend layout decision depends on it).
