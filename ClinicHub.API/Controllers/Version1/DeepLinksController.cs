...
namespace ClinicHub.API.Controllers.Version1
{
    ...
    public class DeepLinksController : BaseApiController
    {
        ...
        private string BuildFallbackPage(string path, bool isAndroid, bool isIos)
        {
            ...
            sb.AppendLine("function toStore(){if(done)return;done=true;clearTimeout(fallbackTimer);if(isMobile){status.textContent='جاري فتح المتجر...';window.location.replace(storeUrl);}else{window.location.replace(storeUrl);}}");
            ...
            sb.AppendLine("btnAndroid.addEventListener('click',toStore);");
            sb.AppendLine("btnIos.addEventListener('click',toStore);");
            sb.AppendLine("btnWeb.addEventListener('click',toStore);");
            ...
            sb.AppendLine("}else{");
            sb.AppendLine("status.textContent='';");
            sb.AppendLine("fallbackTimer=setTimeout(toStore,400);");
            sb.AppendLine("}");
            sb.AppendLine("})();");
            sb.AppendLine("</script>");
            sb.AppendLine("</body>");
