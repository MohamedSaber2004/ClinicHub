...
Controller: `DoctorController` — Views: `Views/Doctor/**`. The doctor identity is hardcoded in the
frontend guard (`CurrentUser` Id=2, ClinicId=`MockData.ClinicId_Heart`); the backend must resolve
the doctor from the token.

### 5.1 ✅ Integrated

| Page | Route | API consumed by the frontend |
|------|-------|------------------------------|
| Availability (schedule) | `/Doctor/Availability` | `GET /admin/dashboard/doctors/availability` → flat rows (dayOfWeek 0–6, startTime/endTime `HH:mm:ss`, slotDurationMinutes, id) · `PUT /admin/dashboard/doctors/availability/week` with `days[]` (existing rows keep id, new rows omit it, removed rows soft-deleted). Per-row duration input (1–480), typical-duration badge = most frequent value recomputed live, empty schedule → seeded default rows Sun–Thu 09:00–17:00 (30 min, no ids), re-render from save response. |

### 5.2 ⚠️ Not implemented — what the backend must add (4 of 5 pages are mock)

| Page | Route | Current state | Backend endpoint(s) needed |
|------|-------|---------------|-----------------------------|
| **Dashboard (Home)** | `/Doctor/Index` | ⚠️ 100% mock | `GET /doctors/dashboard/stats` (today's appointments, patients, next appointment…) + `GET /doctors/appointments?upcoming=true` (next 5) — stats cards + upcoming list. |
| **Appointments** | `/Doctor/Appointments` | ⚠️ 100% mock | `GET /doctors/appointments` (paged, status filter: pending/confirmed/cancelled/completed) + status actions (`PUT /doctors/appointments/{id}/approve|complete|…`). |
| **Patients** | `/Doctor/Patients` | ⚠️ 100% mock | `GET /doctors/patients` (paged, searchTerm) → patient cards (name, phone, last visit, total visits). |
| **Patient History** | `/Doctor/PatientHistory/{patientId}` | ⚠️ 100% mock | `GET /doctors/patients/{id}/history` → visit history + notes/documents per visit. |
