...
- [x] `PUT /admin/clinics/{id}` accepts + persists `latitude` / `longitude`
      (implemented: `UpdateClinicCommand`, validator, handler)
- [x] `GET /admin/clinics/{id}` returns `lat` / `lng` for the saved location
      (already supported via `ClinicManagementProfile`)
- [x] Sending an update **without** `latitude` / `longitude` keeps the existing coordinates unchanged
