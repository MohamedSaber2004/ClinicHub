...
**Register the token with the backend at every login** — the server only registers when **both** fields are present:

```js
const body = { email, password };
const fcmToken = await getFcmToken();
if (fcmToken) {
  body.fcmToken = fcmToken;   // MUST be a non-empty string — never send ""
  body.devicePlatform = 0;    // MUST be a NUMBER 0 = Web (never the string "0")
}
await fetch("https://your-api/api/v1/auth/login-web", {
  method: "POST",
...
Browsers rotate FCM tokens over time and after service-worker updates. If you receive a **new token** from `getToken()` on app start, or push delivery stops while the SW looks fine, re-send `fcmToken` + `devicePlatform: 0` to `POST /api/v1/auth/login-web` (the server replaces the old web token).
...
- [ ] Firebase web app created; VAPID key generated (Console → Project settings → Cloud Messaging).
- [ ] `firebase-messaging-sw.js` served at **`/firebase-messaging-sw.js`** (domain root) with correct `messagingSenderId` + `onBackgroundMessage` + `notificationclick` → opens `data.link`.
- [ ] `getToken(messaging, { vapidKey })` called only after permission granted, **on HTTPS**.
- [ ] Token sent at **every login** (`POST /api/v1/auth/login-web`): `fcmToken` **non-empty string** + `devicePlatform` **number 0**. Verify in DevTools → Network that both fields are in the payload.
- [ ] `onMessage` foreground handler → toast + navigate by `data.link`.
- [ ] Token refresh: re-send the token whenever `getToken()` returns a new one.

### B. In-app bell
- [ ] On dashboard load: `GET /api/v1/notifications/count` → badge, then `GET /api/v1/notifications/pagginated?pageNumber=1&pageSize=20` → list.
- [ ] **Order matters:** the list endpoint marks items as read server-side — fetch the list first, then refresh the badge; or refresh the badge after opening the bell.
- [ ] Infinite scroll: increment `pageNumber` until `hasNextPage = false` (pageSize max 100).
- [ ] Display `titleAr`/`bodyAr` (Arabic) — `titleEn`/`bodyEn` are currently stored empty.
- [ ] Render deep link from `data.link` / navigate by `type` when clicked.
...
|---|---|
| Login succeeds but no `UserFbToken` row (`DevicePlatform = 0`) | Payload missing `fcmToken` or `devicePlatform` (or `devicePlatform` sent as string). See §3.3 — both are required |
