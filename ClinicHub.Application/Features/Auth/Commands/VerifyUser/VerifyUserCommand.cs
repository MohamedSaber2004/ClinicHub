...
namespace ClinicHub.Application.Features.Auth.Commands.VerifyUser
{
    ...
    public class VerifyUserCommandValidator : AbstractValidator<VerifyUserCommand>
    {
        ...
        public VerifyUserCommandValidator(IStringLocalizer<Messages> localizer,UserManager<ApplicationUser> userManager)
        {
            ...
            RuleFor(x => x)
                ...
                {
                    if(x.Email is null) 
                        context.AddFailure(nameof(x.Email), JsonLocalizationProvider.GetLocalizedString(localizer[LocalizationKeys.ValidationMessages.Required.Value]));
